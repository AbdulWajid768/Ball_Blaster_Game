...
public class FireSpeedPopUp extends AbstractPopUp {

    public FireSpeedPopUp(JFrame frame) {
        this.frame = frame;
        initComponents();
    ...
    void initComponents() {
        ...
        try {
            ...
            levelUpCost = rs.getInt("coinstonextfsl");
            maxLevel = rs.getInt("maxfsl");
        } catch (Exception e) {
            e.printStackTrace();
        }
        checkLevelLimit();
    ...
    void upBtnActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        try {
            ...
            DBHandler.stmt.execute("update playerstats set fsl = " + level + " where id = 1");
            DBHandler.stmt.execute("update playerstats set coins = " + coins + " where id = 1");
            levelUpCost = level * level * 10 + 10;
            DBHandler.stmt.execute("update playerstats set coinstonextfsl = " + levelUpCost + " where id = 1");
